...
export async function GET(request: Request): Promise<Response> {
  ...
  const storedState = allCookies.get("github_oauth_state")?.value ?? null;

  if (error) {
    if (error === "access_denied") return redirect("/sign-in");
    return redirect(
      `/sign-in?error=${error}&error_description=${error_description}`,
    );
  }
